...
$image=imagecreatefromjpeg("formatcss.jpg");
...
if ($idcert == $idUser && $namecert == 'css') {
  header('Location: /kidumskill/courses.php');
}
else {
  imagettftext($image, 48, 0, 120, 520, $color,$font, $result);
  imagejpeg($image,"certificate/$vkey_cert.png");//Storing certificate here
  imagedestroy($image);
  $insert1 = $con->query("INSERT INTO cert (user_id,name,vkey) VALUES ('$idUser','css','$vkey_cert')");
