    ...
    <div class="row scroll ">
      <div class="exme">
        <form  action="" method="post" class="exme_card" dir="rtl">
          <?php
            for ($e=$fromq; $e < $toq ; $e++) {
            $qe = "select * from questions_phpmysql where qid = $e";
            ...
            while ($rowse = mysqli_fetch_array($querye)) { ?>
              <h4 class="card-headerr">
                <?php echo $rowse['question']; ?>
                        ...
                        <?php
            $qe = "select * from answers_phpmysql where ans_id =$e";
            $querye = mysqli_query($cone, $qe);
            while ($rowse = mysqli_fetch_array($querye) ){
            ?>
...
<div class="check">
          ...
          <?php $result=0;
                ...
                // print_r($selected);
                $qe = "select * from questions_phpmysql";
                $querye = mysqli_query($cone, $qe);
                while($rowse = mysqli_fetch_array($querye)) {
...
<?php           }elseif ($result == 8) {?>
  <div class="success_green">
            <?php    echo "עברתם ";?>
              ...
              <?php
                echo "<p>80</p>";
                $mysqli->query("UPDATE users SET phpmysql = phpmysql+1 WHERE `username` = '".$username."' "); ?>
                </div>
...
<?php           }else { ?>
          ...
          <?php        $mysqli->query("UPDATE users SET phpmysql = phpmysql+1 WHERE `username` = '".$username."' ");
                }
                     ?>
                     ...
                     <div class="again start">
                         <input type="button" value="לנסות שוב" onclick="history.back(-1)" class="btn btn-success" />
                            ...
                            <form  action="" method="post">
                                ...
                                <a href="phpmysql.php?item_id=<?php echo $item["id"]+1; ?>" class="next" id="next">המשך
                                </a>
