<div class="panel-right">
  <div class="lessons scroll">
      ...
      <!--  -->
      <?php
  $itemList = $rating->getHtmlList();


  if ($item["id"] > $exme1 && $block == 0 && $username = $_COOKIE['user']){
    echo "<script type='text/javascript'>alert('1צריך לעשות מבחן כדי להתקדם'); history.back(-1)</script>   ";
 }elseif ($item["id"] > $exme2 && $block == 1 && $username = $_COOKIE['user']) {
   echo "<script type='text/javascript'>alert('2צריך לעשות מבחן כדי להתקדם'); history.back(-1)</script>   ";
 }elseif ($item["id"] > $exme3 && $block == 2 && $username = $_COOKIE['user']) {
   echo "<script type='text/javascript'>alert('3צריך לעשות מבחן כדי להתקדם'); history.back(-1)</script>   ";
 }
...
$current_page = str_replace('kidumskill/incCourses/html/html.php?item_id=', '', $current_page);
 ?>
  ...
  <div class="container" >
    <div class="row">
      <div class="lesson active">
        ...
        <a href="html.php?item_id=<?php echo $item["id"]; ?>" style="text-decoration: none;" >
